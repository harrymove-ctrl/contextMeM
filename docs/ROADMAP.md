...
- **No smart contract.** `find` for `Move.toml`/`*.move` returns nothing. Every "on-chain" touch is either a **read-only Sui query** (`SuiGrpcClient.getObject` in `proof.ts`/`resources.ts`, raw JSON-RPC in `history.ts`, SuiNS `nameService.lookupName` in `resolve.ts`, and BCS deserialization schemas in `bcs.ts` — all reading *Mysten's* Walrus Sites package) or a **write mediated by a third party** (Tatum signs the Walrus tx; MemWal's relayer holds memory). ContextMEM never signs a Sui tx today.
...
- **What's thin:** no hosted accounts (identity = MemWal delegate in browser `localStorage`); no billing/metering; no observability; rate-limiting only on the demo IP quota; CI is typecheck+test+build; **a fully-built namespace management console (`NamespacesAppPage`, `main.tsx:5943`) is never routed.**
...
| Private namespaces via Seal policies | P1 | L | `visibility='private'` → owned Seal policy; granting a read token also grants SEAL decrypt (wire into `CloudflareNamespaceStore.authorizeNamespace`, `worker.ts:502`). Makes "private" real at the chain layer. |
...
### Current state
React 19 + Vite + react-router + three.js; **no Tailwind/shadcn** — one hand-written 10,241-line `styles.css` with `--cm-*` tokens (light theme, brand green `#a8d946`, dark graph `#0f1115`). **Almost the entire app is in one 8,203-line `main.tsx`** (~70 components); the namespace-memory cluster (constellation, search box, legend, chunk panel) plus a few block/react-bits components are split into `components/`, with shared helpers in `lib/`. Routes: landing (parallax hero + live demo extraction), `/share/:shareId`, `/showcase`, and gated `/app/*` (Build, Artifacts, Runs, Memory, Compare, Publish, Namespaces, Settings). 3D "memory constellation" (OrbitControls autorotate, bloom, fibonacci-sphere, DOM labels, search, detail panel). Grounded chat panel (multi-turn, per-turn sources/confidence). Non-delegate users are hard-gated behind `LockedPreview`/`HostedCredentialGate`.
